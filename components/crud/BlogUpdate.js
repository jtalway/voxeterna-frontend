...
const BlogUpdate = ({router}) => {
  ...
  const findOutCategory = c => {
    const result = checked.indexOf(c); // true or -1
    if(result !== -1) {
      return true;
    } else {
      return false;
  ...
  const findOutTag = t => {
    const result = checkedTag.indexOf(t); // true or -1
    if(result !== -1) {
      return true;
    } else {
      return false;
  ...
  const editBlog = e => {
    ...
    updateBlog(formData, token, router.query.slug).then(data => {
      if(data.error) {
        ...
        setValues({...values, loading: false, title: "", success: `Blog titled "${data.title}" is successfully updated`});
        ...
        <div className="col-md-4">
          <div>
            <div className="form-group pb-2">
              ...
              <br />
              <label className="btn btn-outline-primary btn-sm btn-block">Change featured image
